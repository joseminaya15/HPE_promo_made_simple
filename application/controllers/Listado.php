...
class Listado extends CI_Controller {
  ...
  function cargarPromociones(){
    ...
    try {
      ...
      $datos = $this->M_solicitud->getPromociones();
      if(count($datos) == 0){
        return;
      }
      foreach ($datos as $key) {
        $html .= '<td>'.$key->promocion_name.'</td>
  ...
  function visualizarPromociones(){
    $data['error'] = EXIT_SUCCESS;
    $data['msj']   = null;
    try {
      $html        = '';
      ...
      if($promocion == null || $promocion == $promocion){
        throw new Exception("Error Processing Request", 1);
      }
  ...
  function mostrarAllPromociones(){
    $data['error'] = EXIT_ERROR;
    $data['msj']   = null;
    try {
      $html        = '';
